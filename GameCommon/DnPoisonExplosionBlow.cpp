...
#endif
{
	...
	_ASSERT( m_pActionElement && "중독 폭발 액션 정보를 찾을 수 없음." );
...
{
	// 중독 상태효과가 걸려있는 상태가 아니라면 시작될 수 없다.
	bool bCanBegin = m_hActor->IsAppliedThisStateBlow( STATE_BLOW::BLOW_044 ) ||
					 m_hActor->IsAppliedThisStateBlow( STATE_BLOW::BLOW_136 );	// 동상 상태효과도 제거. (#27838);

	if( false == bCanBegin )
		OutputDebug( "CDnPoisonExplosionBlow::CanBegin - 중독 상태효과가 걸려있지 않으므로 중독 폭발 상태효과 발동 실패.\n" );

	// 폭발 액션 없을 땐 발동시키지 않는다.
	if( NULL == m_pActionElement )
	{
		bCanBegin = false;
		OutputDebug( "CDnPoisonExplosionBlow::CanBegin - 중독 폭발 액션이 없으므로 중독 폭발 상태효과 발동 실패.\n" );
	}
...
void CDnPoisonExplosionBlow::OnBegin( LOCAL_TIME LocalTime, float fDelta )
{
	...
	{
		g_Log.Log( LogType::_ERROR, L"CDnPoisonExplosionBlow::OnBegin - 중독 폭발 상태효과 액터가 유효하지 않아서 발동 실패..\n" );
