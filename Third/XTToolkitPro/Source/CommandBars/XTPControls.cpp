...
CSize CXTPControls::_CalcSize(XTPBUTTONINFO* pData, BOOL bVert)
{
	...
	const int nWrapMargin = pData->nWrapMargin;
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		if (bVert)
		{
			...
			if (pControl->bWrap)
			{
				cur.x -= nRow + (pControl->bBeginGroup ? szSeparators.cy : nWrapMargin);
				nRow = cur.y = 0;
			}
		...
		else
		{
			...
			if (pControl->bWrap)
			{
				cur.y += nRow + (pControl->bBeginGroup ? szSeparators.cy : nWrapMargin);
				nRow = cur.x = 0;
			}
...
int CXTPControls::_WrapToolBar(XTPBUTTONINFO* pData, int nWidth, DWORD& dwMode)
{
	...
	BOOL bFirst = TRUE;
	int nSeparator = bVert ? pData->szSeparators.cy : pData->szSeparators.cx;
...
CSize CXTPControls::CalcDynamicSize(CDC* pDC, int nLength, DWORD dwMode, const CRect& rcBorder, int nWidth)
{
	...
	if (GetVisibleCount() > 0)
	{
		...
		pData->nWrapMargin = m_pParent->m_nWrapMargin;
