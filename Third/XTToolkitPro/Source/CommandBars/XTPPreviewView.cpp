...
CXTPPreviewView::CXTPPreviewView()
{
	...
	m_cxPixelsPerInch = GetDeviceCaps(hDCScreen, LOGPIXELSX);
	m_cyPixelsPerInch = GetDeviceCaps(hDCScreen, LOGPIXELSY);
...
int CXTPPreviewView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPControlPopup* pTwoPages = (CXTPControlPopup*)m_wndToolBar.GetControls()->SetControlType(FindToolbarButton(XTP_ID_PREVIEW_TWOPAGE), xtpControlButtonPopup);
	...
	CXTPControlEdit* pControlGotoPage = (CXTPControlEdit*)m_wndToolBar.GetControls()->SetControlType(FindToolbarButton(XTP_ID_PREVIEW_GOTOPAGE), xtpControlEdit);

	pControlGotoPage->SetWidth(100);

	CXTPControlComboBox* pControlZoom = (CXTPControlComboBox*)m_wndToolBar.GetControls()->SetControlType(FindToolbarButton(XTP_ID_PREVIEW_ZOOMCOMBO), xtpControlComboBox);
...
{
	return m_wndToolBar.GetControls()->FindControl(nCommandID);
}
...
void CXTPPreviewView::OnLButtonDown(UINT, CPoint point)
{
	...
	if (m_nZoomState == XPT_ZOOM_WHOLEPAGE)
	{
...
void CXTPPreviewView::SetZoomState(UINT nNewState, UINT nPage, CPoint point)
{
	if (m_nZoomState != nNewState)
	{
		...
		if (nNewState == XPT_ZOOM_WHOLEPAGE)
		{
			pControlCombo->SetCurSel(0);
		}
		else
		{
			for (int i = 0; i < pControlCombo->GetCount(); i++)
			{
				if (pControlCombo->GetItemData(i) == nNewState)
				{
					pControlCombo->SetCurSel(i);
					break;
				}
			}
		}
...
void CXTPPreviewView::SetScaledSize(UINT nPage)
{
	...
	switch (m_nZoomState)
	{
		case XPT_ZOOM_WHOLEPAGE:
			*pRatio = *pZoomOutRatio;
			break;

		case XPT_ZOOM_PAGEWIDTH:
			m_Across = 1;
			m_Down = 1;
			m_nPages = 1;
...
void CXTPPreviewView::OnUpdateNumPageChange(CCmdUI* /*pCmdUI*/)
{
	...
	//UINT nPages = m_nZoomState == XPT_ZOOM_WHOLEPAGE ? m_nPages : m_nZoomOutPages;
	//pCmdUI->Enable(m_nZoomState == XPT_ZOOM_WHOLEPAGE && m_nMaxPages != 1 && (m_pPreviewInfo->GetMaxPage() > 1 || m_nPages > 1));
...
{
	if (m_nZoomState == XPT_ZOOM_WHOLEPAGE || m_nZoomState == XPT_ZOOM_PAGEWIDTH)
	{
	...
	// return size of current page less margins
{
	...
	if (m_nZoomState == XPT_ZOOM_WHOLEPAGE && (m_pPreviewInfo->GetMaxPage() != 0xffff) && (m_pPreviewInfo->GetMaxPage() - m_pPreviewInfo->GetMinPage() != 0))
...
void CXTPPreviewView::OnZoomCombo(NMHDR* pNMHDR, LRESULT* pRes)
{
	...
	if (nCurSel != -1)
	{
		if (nCurSel == XPT_ZOOM_WHOLEPAGE || nCurSel == XPT_ZOOM_PAGEWIDTH)
		{
...
void CXTPPreviewView::OnOnePage()
{
	...
	SetZoomState(XPT_ZOOM_WHOLEPAGE, 0, CPoint(0, 0));
	if (m_nZoomState == XPT_ZOOM_WHOLEPAGE)
	{
		// make sure the current page does not cause blank pages to be shown
		if (m_nCurrentPage >= m_pPreviewInfo->GetMaxPage() - m_nPages)
...
void CXTPPreviewView::OnSize(UINT nType, int cx, int cy)
{
	...
	else
	{
		...
		if (m_nZoomState == XPT_ZOOM_PAGEWIDTH)
		{
			// Force recalc of scale ratios on next draw
			for (UINT i = 0; i < m_nMaxPages; i++)
				m_pPageInfo[i].sizeScaleRatio.cx = 0;           // zero scale ratios
