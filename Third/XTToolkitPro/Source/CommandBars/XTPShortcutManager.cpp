...
{
	delete m_arrAccels[nIndex];
	m_arrAccels.RemoveAt(nIndex);
}

void CXTPShortcutManagerAccelTable::Add(CXTPShortcutManagerAccel accel)
{
	m_arrAccels.Add(new CXTPShortcutManagerAccel(accel));
}
...
void CXTPShortcutManagerAccelTable::CopyAccelTable(LPACCEL lpAccel, int nSize)
{
	...
	{
		m_arrAccels.Add(new CXTPShortcutManagerAccel(&lpAccel[i]));
	}
}

CXTPShortcutManagerAccel* CXTPShortcutManagerAccelTable::CopyAccels() const
{
	CXTPShortcutManagerAccel* pAccels = new CXTPShortcutManagerAccel[GetCount()];
...
{
	return m_arrAccels.GetAt(nIndex);
}
...
void CXTPShortcutManagerAccelTable::CopyAccelTable(CXTPShortcutManagerAccelTable* pAccelTable)
{
	...
	{
		m_arrAccels.Add(new CXTPShortcutManagerAccel(*pAccelTable->GetAt(i)));
	}
}
...
CXTPShortcutManager::CXTPShortcutManager(CXTPCommandBars* pCommandBars)
{
	...
	m_pAccelTable = new CXTPShortcutManagerAccelTable();
...
void CXTPShortcutManager::CreateOriginalAccelTable()
{
	if (m_pOriginalAccelTable == NULL)
		m_pOriginalAccelTable = new CXTPShortcutManagerAccelTable();
...
BOOL CXTPShortcutManager::TranslateAccelerator(LPMSG lpMsg)
{
	...
	for (int i = 0; i < nAccelSize; i++)
	{
		...
		if (IsWindowEnabled(hWnd) && ::GetCapture() == NULL && !IsIconic(hWnd) && accel->cmd != 0)
		{
			if (accel->key[1].key != 0)
			{
				if (m_pCommandBars->GetSite()->IsFrameWnd())
				{
					...
					VERIFY(XTPResourceManager()->LoadString(&strFormat, XTP_IDS_SHORTCUT_SECONDKEY));
...
BOOL CXTPShortcutManager::FindDefaultAccelerator(int nCmd, CString& strShortcut)
{
	...
	if (m_pOriginalAccelTable == NULL && pAccelTable == NULL)
		return FindDefaultFrameAccelerator(nCmd, strShortcut);

	if (!pAccelTable || pAccelTable->GetCount() == 0)
		return FALSE;
...
{
	memset(&m_accel, 0, sizeof(CXTPShortcutManagerAccel));
	m_nKeyDefined = 0;
...
void CXTPShortcutManager::UpdateAcellTable(CXTPShortcutManagerAccel* lpAccel, int nSize)
{
	m_pAccelTable->RemoveAll();
...
void CXTPShortcutManager::SaveShortcuts(LPCTSTR lpszProfileName)
{
	...
	CXTPShortcutManagerAccel* lpAccel = m_pAccelTable->CopyAccels();

	AfxGetApp()->WriteProfileBinary(lpszProfileName,  _T("Accelerators"), (LPBYTE)lpAccel, m_pAccelTable->GetCount() * sizeof(CXTPShortcutManagerAccel));
...
void CXTPShortcutManager::LoadShortcuts(LPCTSTR lpszProfileName)
{
	...
	{
		UINT uiSize;
		CXTPShortcutManagerAccel* lpAccel = 0;
		if (AfxGetApp()->GetProfileBinary(lpszProfileName,  _T("Accelerators"), (LPBYTE*) &lpAccel, &uiSize))
		{
			int nAccelSize = uiSize / sizeof(CXTPShortcutManagerAccel);
			ASSERT (lpAccel != NULL);
...
void CXTPShortcutManager::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		...
		if (pPX->IsAllowBlobValues())
		{
			...
			pPX->Write(_T("Data"), lpAccel, nAccelSize * sizeof(CXTPShortcutManagerAccel));
	...
	else
	{
		...
		else
		{
			...
			{
				int nAccelSize = (int)pPX->ReadCount();
				CXTPShortcutManagerAccel* lpAccel = new CXTPShortcutManagerAccel[nAccelSize];
				pPX->Read(_T("Data"), lpAccel, nAccelSize * sizeof(CXTPShortcutManagerAccel));
