...
BOOL CXTPTaskDialogClient::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (m_pMarkupContext)
	{
		...
		if (XTPMarkupRelayMessage(m_strVerificationText.m_pUIElement, message, wParam, lParam, pResult))
			return TRUE;

		if (XTPMarkupRelayMessage(m_strExpandedInformation.m_pUIElement, message, wParam, lParam, pResult))
			return TRUE;

		if (XTPMarkupRelayMessage(m_strExpandedControlText.m_pUIElement, message, wParam, lParam, pResult))
			return TRUE;

		if (XTPMarkupRelayMessage(m_strCollapsedControlText.m_pUIElement, message, wParam, lParam, pResult))
			return TRUE;
	}
...
void CXTPTaskDialogClient::LoadStrings()
{
	...
	if (!m_strExpandedInformation.IsEmpty())
	{
		...
		{
			VERIFY(XTPResourceManager()->LoadString(&m_strExpandedControlText.m_strCaption, XTP_IDS_TASK_HIDE_DETAILS));
			VERIFY(XTPResourceManager()->LoadString(&m_strCollapsedControlText.m_strCaption, XTP_IDS_TASK_SEE_DETAILS));
		}
...
BOOL CXTPTaskDialogClient::CreateInputBox()
{
	...
	if (!m_wndInputBox.CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), m_strInputBoxText, WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL, CRect(0, 0, 0, 0), this, 0))
		return FALSE;
...
void CXTPTaskDialogClient::DrawContent(CDC* pDC)
{
	...
	{
		DrawText(pDC, m_strContent, m_rcContent, m_crContextText, &m_font);
	}
}
...
void CXTPTaskDialogClient::DrawExpandedInfo(CDC* pDC)
{
	...
	{
		DrawText(pDC, m_strExpandedInformation, m_rcExpandedInformation, IsExpandFooterArea() ? m_crFooterText : m_crContextText, &m_font);
	}
}
