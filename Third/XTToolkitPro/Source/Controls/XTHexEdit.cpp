...
IMPLEMENT_DYNAMIC(CXTHexEdit, CWnd)

BEGIN_MESSAGE_MAP(CXTHexEdit, CXTHexEditBase)
...
LRESULT CXTHexEdit::OnPrintClient(WPARAM wParam, LPARAM /*lParam*/)
{
...
void CXTHexEdit::OnDraw(CDC* pDC)
{
	// Get the client rect.
...
{
	CXTHexEditBase::OnSetFocus(pOldWnd);
...
{
	DestroyCaret();
	CXTHexEditBase::OnKillFocus(pNewWnd);
}
...
{
	if (!CXTHexEditBase::PreCreateWindow(cs))
		return FALSE;
...
void CXTHexEdit::GetSel(int& nSelStart, int& nSelEnd)
{
...
void CXTHexEdit::OnSize(UINT nType, int cx, int cy)
{
	...
	CXTHexEditBase::OnSize(nType, cx, cy);
...
void CXTHexEdit::OnNcPaint()
{
	...
	CXTHexEditBase::OnNcPaint(); // Be sure and call the parent class
