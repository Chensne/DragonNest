...
LRESULT CXTCaption::OnPrintClient(WPARAM wParam, LPARAM /*lParam*/)
{
...
void CXTCaption::SetCaptionColors(COLORREF clrBorder, COLORREF clrFace, COLORREF clrText)
{
	...
	GetTheme()->m_clrBorder.SetCustomValue(clrBorder);
	GetTheme()->m_clrFace.SetCustomValue(clrFace);
	GetTheme()->m_clrText.SetCustomValue(clrText);

	if (::IsWindow(m_btnCaption.m_hWnd))
	{
		m_btnCaption.SetColorFace(GetTheme()->m_clrFace);
		m_btnCaption.SetColorText(GetTheme()->m_clrText);
		m_btnCaption.SetColorShadow(GetTheme()->m_clrEdgeShadow);
	}
}
