...
{
	TRACE(_T("CXTPSyntaxEditCtrl::OnCreate() - %d \n"), ::GetTickCount());
...
{
	//TRACE(_T("CXTPSyntaxEditCtrl::OnDestroy() - %d \n\n"), ::GetTickCount());
...
BOOL CXTPSyntaxEditCtrl::IncreaseIndent()
{
	...
	if (m_Selection.GetEnd_disp().nCol == 1 && iRowTo > iRowFrom)
		iRowTo--;
								  ...
								  int& iChainActionCount)
{
	...
	if (!bTextCRLF)
	{
		if (szText[0] == 0x09 && !GetTabWithSpace() &&
