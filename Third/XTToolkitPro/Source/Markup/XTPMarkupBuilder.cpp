...
CXTPMarkupObject* CXTPMarkupBuilder::CStaticExtension::GetSystemColor(int nIndex)
{
	...
	{
		m_pColors[nIndex] = new CXTPMarkupColor(GetSysColor(nIndex));
	}
...
CXTPMarkupObject* CXTPMarkupBuilder::CStaticExtension::GetSystemBrush(int nIndex)
{
	...
	{
		m_pBrushes[nIndex] = new CXTPMarkupSolidColorBrush(GetSysColor(nIndex));
	}
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertValue(CXTPMarkupDependencyProperty* pProperty, CXTPMarkupObject* pValue)
{
	...
	{
		pNewValue = (*pProperty->GetMetadata()->m_pConverter)(GetMarkupContext(), pValue);
	}

	if (pNewValue == NULL)
	{
		CXTPMarkupObject* pConverter = GetMarkupContext()->CreateMarkupObject(pProperty->GetPropetyType());
		pNewValue = pConverter->ConvertFrom(pValue);
		MARKUP_RELEASE(pConverter);
	}
			...
			case CXTPMarkupParser::tokenTagEnd:
				{
					...
					if (pRootElement->IsKindOf(MARKUP_TYPE(CXTPMarkupObjectProperty)))
					{
						...
						m_arrObjects.GetTail()->SetPropertyObject(this, pObjectProperty->m_pProperty, pObjectProperty->m_pContent);
...
BOOL CXTPMarkupBuilder::ColorStringToKnownColor(LPCWSTR lpszValue, COLORREF& clr)
{
	...
	{
	case 3:
		if (ch == 'R' && _wcsicmp(lpszValue, L"RED") == 0) return clr = 0x0000ff;
		if (ch == _T('T') && _wcsicmp(lpszValue, L"TAN") == 0) return clr = 0x8cb4d2;
		return FALSE;
	case 4:
		switch (ch)
		{
		case _T('A'): if (_wcsicmp(lpszValue, L"AQUA") == 0) return clr = 0xffff00; return FALSE;
		case _T('B'): if (_wcsicmp(lpszValue, L"BLUE") == 0) return clr = 0xff0000; return FALSE;
		case _T('C'): if (_wcsicmp(lpszValue, L"CYAN") == 0) return clr = 0xffff00; return FALSE;
		case _T('G'):
			if (_wcsicmp(lpszValue, L"GOLD") == 0) return clr = 0x00d7ff;
			if (_wcsicmp(lpszValue, L"GRAY") == 0) return clr = 0x808080;
			return FALSE;
		case _T('L'): if (_wcsicmp(lpszValue, L"LIME") == 0)clr = 0x00ff00; return FALSE;
		case _T('N'): if (_wcsicmp(lpszValue, L"NAVY") == 0)return clr = 0x800000; return FALSE;
		case _T('P'):
			if (_wcsicmp(lpszValue, L"PERU") == 0) return clr = 0x3f85cd;
			if (_wcsicmp(lpszValue, L"PINK") == 0) return clr = 0xcbc0ff;
			if (_wcsicmp(lpszValue, L"PLUM") == 0) return clr = 0xdda0dd;
			return FALSE;
		case _T('S'): if (_wcsicmp(lpszValue, L"SNOW") == 0) return clr = 0xfafaff; return FALSE;
		case _T('T'): if (_wcsicmp(lpszValue, L"TEAL") == 0) return clr = 0x808000; return FALSE;
		}
		return FALSE;
	case 5:
		switch (ch)
		{
		case _T('A'): if (_wcsicmp(lpszValue, L"AZURE") == 0) return clr = 0xfffff0; return FALSE;
		case _T('B'):
			if (_wcsicmp(lpszValue, L"BEIGE") == 0) return clr = 0xdcf5f5;
			if (_wcsicmp(lpszValue, L"BLACK") == 0) { clr = 0; return TRUE; }
			if (_wcsicmp(lpszValue, L"BROWN") == 0) return clr = 0x2a2aa5;
			return FALSE;
		case _T('C'): if (_wcsicmp(lpszValue, L"CORAL") == 0) return clr = 0x507fff; return FALSE;
		case _T('G'): if (_wcsicmp(lpszValue, L"GREEN") == 0) return clr = 0x008000; return FALSE;
		case _T('I'): if (_wcsicmp(lpszValue, L"IVORY") == 0) return clr = 0xf0ffff; return FALSE;
		case _T('K'): if (_wcsicmp(lpszValue, L"KHAKI") == 0) return clr = 0x8ce6f0; return FALSE;
		case _T('L'): if (_wcsicmp(lpszValue, L"LINEN") == 0) return clr = 0xe6f0fa; return FALSE;
		case _T('O'): if (_wcsicmp(lpszValue, L"OLIVE") == 0) return clr = 0x008080; return FALSE;
		case _T('W'):
			if (_wcsicmp(lpszValue, L"WHEAT") == 0) return clr = 0xb3def5;
			if (_wcsicmp(lpszValue, L"WHITE") == 0) return clr = 0xffffff;
			return FALSE;
		}
		return FALSE;
	case 6:
		switch (ch)
		{
		case _T('M'): if (_wcsicmp(lpszValue, L"MAROON") == 0) return clr = 0x000080; return FALSE;
		case _T('O'):
			if (_wcsicmp(lpszValue, L"ORANGE") == 0) return clr = 0x00a5ff;
			if (_wcsicmp(lpszValue, L"ORCHID") == 0) return clr = 0xd670da;
			return FALSE;
		case _T('P'): if (_wcsicmp(lpszValue, L"PURPLE") == 0) return clr = 0x800080; return FALSE;
		case _T('S'):
			if (_wcsicmp(lpszValue, L"SALMON") == 0) return clr = 0x7280fa;
			if (_wcsicmp(lpszValue, L"SIENNA") == 0) return clr = 0x2d52a0;
			if (_wcsicmp(lpszValue, L"SILVER") == 0) return clr = 0xc0c0c0;
			return FALSE;
		case _T('T'): if (_wcsicmp(lpszValue, L"TOMATO") == 0) return clr = 0x4763ff; return FALSE;
		case _T('V'): if (_wcsicmp(lpszValue, L"VIOLET") == 0) return clr = 0xee82ee; return FALSE;
		case _T('Y'): if (_wcsicmp(lpszValue, L"YELLOW") == 0) return clr = 0x00ffff; return FALSE;
		case _T('I'): if (_wcsicmp(lpszValue, L"INDIGO") == 0) return clr = 0x82004b; return FALSE;
		case _T('B'): if (_wcsicmp(lpszValue, L"BISQUE") == 0) return clr = 0xc4e4ff; return FALSE;
		}
		return FALSE;
	case 7:
		switch (ch)
		{
		case _T('C'): if (_wcsicmp(lpszValue, L"CRIMSON") == 0) return clr = 0x3c14dc; return FALSE;
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKRED") == 0) return clr = 0x00008b;
			if (_wcsicmp(lpszValue, L"DIMGRAY") == 0) return clr = 0x696969;
			return FALSE;
		case _T('F'): if (_wcsicmp(lpszValue, L"FUCHSIA") == 0) return clr = 0xff00ff; return FALSE;
		case _T('H'): if (_wcsicmp(lpszValue, L"HOTPINK") == 0) return clr = 0xb469ff; return FALSE;
		case _T('M'): if (_wcsicmp(lpszValue, L"MAGENTA") == 0) return clr = 0xff00ff; return FALSE;
		case _T('O'): if (_wcsicmp(lpszValue, L"OLDLACE") == 0)return clr = 0xe6f5fd; return FALSE;
		case _T('S'): if (_wcsicmp(lpszValue, L"SKYBLUE") == 0) return clr = 0xebce87; return FALSE;
		case _T('T'): if (_wcsicmp(lpszValue, L"THISTLE") == 0) return clr = 0xd8bfd8; return FALSE;
		}
		return FALSE;
	case 8:
		switch (ch)
		{
		case _T('C'): if (_wcsicmp(lpszValue, L"CORNSILK") == 0) return clr = 0xdcf8ff; return FALSE;
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKBLUE") == 0) return clr = 0x8b0000;
			if (_wcsicmp(lpszValue, L"DARKCYAN") == 0) return clr = 0x8b8b00;
			if (_wcsicmp(lpszValue, L"DARKGRAY") == 0) return clr = 0xa9a9a9;
			if (_wcsicmp(lpszValue, L"DEEPPINK") == 0) return clr = 0x9314ff;
			return FALSE;
		case _T('H'): if (_wcsicmp(lpszValue, L"HONEYDEW") == 0) return clr = 0xf0fff0; return FALSE;
		case _T('L'): if (_wcsicmp(lpszValue, L"LAVENDER") == 0) return clr = 0xfae6e6; return FALSE;
		case _T('M'): if (_wcsicmp(lpszValue, L"MOCCASIN") == 0) return clr = 0xb5e4ff; return FALSE;
		case _T('S'):
			if (_wcsicmp(lpszValue, L"SEAGREEN") == 0) return clr = 0x578b2e;
			if (_wcsicmp(lpszValue, L"SEASHELL") == 0) return clr = 0xeef5ff;
			return FALSE;
		}
		return FALSE;
	case 9:
		switch (ch)
		{
		case _T('A'): if (_wcsicmp(lpszValue, L"ALICEBLUE") == 0) return clr = 0xfff8f0; return FALSE;
		case _T('B'): if (_wcsicmp(lpszValue, L"BURLYWOOD") == 0) return clr = 0x87b8de; return FALSE;
		case _T('C'):
			if (_wcsicmp(lpszValue, L"CADETBLUE") == 0) return clr = 0xa09e5f;
			if (_wcsicmp(lpszValue, L"CHOCOLATE") == 0) return clr = 0x1e69d2;
			return FALSE;
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKGREEN") == 0) return clr = 0x006400;
			if (_wcsicmp(lpszValue, L"DARKKHAKI") == 0) return clr = 0x6bb7bd;
			return FALSE;
		case _T('F'): if (_wcsicmp(lpszValue, L"FIREBRICK") == 0) return clr = 0x2222b2; return FALSE;
		case _T('G'):
			if (_wcsicmp(lpszValue, L"GAINSBORO") == 0) return clr = 0xdcdcdc;
			if (_wcsicmp(lpszValue, L"GOLDENROD") == 0) return clr = 0x20a5da;
			return FALSE;
		case _T('I'): if (_wcsicmp(lpszValue, L"INDIANRED") == 0)return clr = 0x5c5ccd; return FALSE;
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LAWNGREEN") == 0)return clr = 0x00fc7c;
			if (_wcsicmp(lpszValue, L"LIGHTBLUE") == 0) return clr = 0xe6d8ad;
			if (_wcsicmp(lpszValue, L"LIGHTCYAN") == 0) return clr = 0xffffe0;
			if (_wcsicmp(lpszValue, L"LIGHTGRAY") == 0) return clr = 0xd3d3d3;
			if (_wcsicmp(lpszValue, L"LIGHTPINK") == 0) return clr = 0xc1b6ff;
			if (_wcsicmp(lpszValue, L"LIMEGREEN") == 0) return clr = 0x32cd32;
			return FALSE;
		case _T('M'):
			if (_wcsicmp(lpszValue, L"MINTCREAM") == 0) return clr = 0xfafff5;
			if (_wcsicmp(lpszValue, L"MISTYROSE") == 0) return clr = 0xe1e4ff;
			return FALSE;
		case _T('O'):
			if (_wcsicmp(lpszValue, L"OLIVEDRAB") == 0) return clr = 0x238e6b;
			if (_wcsicmp(lpszValue, L"ORANGERED") == 0) return clr = 0x0045ff;
			return FALSE;
		case _T('P'):
			if (_wcsicmp(lpszValue, L"PALEGREEN") == 0) return clr = 0x98fb98;
			if (_wcsicmp(lpszValue, L"PEACHPUFF") == 0) return clr = 0xb9daff;
			return FALSE;
		case _T('R'):
			if (_wcsicmp(lpszValue, L"ROSYBROWN") == 0) return clr = 0x8f8fbc;
			if (_wcsicmp(lpszValue, L"ROYALBLUE") == 0) return clr = 0xe16941;
			return FALSE;
		case _T('S'):
			if (_wcsicmp(lpszValue, L"SLATEBLUE") == 0) return clr = 0xcd5a6a;
			if (_wcsicmp(lpszValue, L"SLATEGRAY") == 0)  return clr = 0x908070;
			if (_wcsicmp(lpszValue, L"STEELBLUE") == 0) return clr = 0xb48246;
			return FALSE;
		case _T('T'): if (_wcsicmp(lpszValue, L"TURQUOISE") == 0) return clr = 0xd0e040; return FALSE;
		}
		return FALSE;
	case 10:
		switch (ch)
		{
		case _T('A'): if (_wcsicmp(lpszValue, L"AQUAMARINE") == 0) return clr = 0xd4ff7f; return FALSE;
		case _T('B'): if (_wcsicmp(lpszValue, L"BLUEVIOLET") == 0) return clr = 0xe22b8a; return FALSE;
		case _T('C'): if (_wcsicmp(lpszValue, L"CHARTREUSE") == 0) return clr = 0x00ff7f; return FALSE;
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKORANGE") == 0) return clr = 0x008cff;
			if (_wcsicmp(lpszValue, L"DARKORCHID") == 0) return clr = 0xcc3299;
			if (_wcsicmp(lpszValue, L"DARKSALMON") == 0) return clr = 0x7a96e9;
			if (_wcsicmp(lpszValue, L"DARKVIOLET") == 0) return clr = 0xd30094;
			if (_wcsicmp(lpszValue, L"DODGERBLUE") == 0) return clr = 0xff901e;
			return FALSE;
		case _T('G'): if (_wcsicmp(lpszValue, L"GHOSTWHITE") == 0) return clr = 0xfff8f8; return FALSE;
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LIGHTCORAL") == 0)  return clr = 0x8080f0;
			if (_wcsicmp(lpszValue, L"LIGHTGREEN") == 0) return clr = 0x90ee90;
			return FALSE;
		case _T('M'):
			if (_wcsicmp(lpszValue, L"MEDIUMBLUE") == 0) return clr = 0xcd0000; return FALSE;
		case _T('P'):
			if (_wcsicmp(lpszValue, L"PAPAYAWHIP") == 0) return clr = 0xd5efff;
			if (_wcsicmp(lpszValue, L"POWDERBLUE") == 0) return clr = 0xe6e0b0;
			return FALSE;
		case _T('S'): if (_wcsicmp(lpszValue, L"SANDYBROWN") == 0) return clr = 0x60a4f4; return FALSE;
		case _T('W'): if (_wcsicmp(lpszValue, L"WHITESMOKE") == 0) return clr = 0xf5f5f5; return FALSE;
		}
		return FALSE;
	case 11:
		switch (ch)
		{
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKMAGENTA") == 0) return clr = 0x8b008b;
			if (_wcsicmp(lpszValue, L"DEEPSKYBLUE") == 0) return clr = 0xffbf00;
			return FALSE;
		case _T('F'):
			if (_wcsicmp(lpszValue, L"FLORALWHITE") == 0)  return clr = 0xf0faff;
			if (_wcsicmp(lpszValue, L"FORESTGREEN") == 0) return clr = 0x228b22;
			return FALSE;
		case _T('G'): if (_wcsicmp(lpszValue, L"GREENYELLOW") == 0) return clr = 0x2fffad; return FALSE;
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LIGHTSALMON") == 0)  return clr = 0x7aa0ff;
			if (_wcsicmp(lpszValue, L"LIGHTYELLOW") == 0) return clr = 0xe0ffff;
			return FALSE;
		case _T('N'):
			if (_wcsicmp(lpszValue, L"NAVAJOWHITE") == 0) return clr = 0xaddeff;
			return FALSE;
		case _T('S'):
			if (_wcsicmp(lpszValue, L"SADDLEBROWN") == 0) return clr = 0x13458b;
			if (_wcsicmp(lpszValue, L"SPRINGGREEN") == 0) return clr = 0x7fff00;
			return FALSE;
		case _T('T'): if (_wcsicmp(lpszValue, L"TRANSPARENT") == 0) return 0; return FALSE;
		case _T('Y'): if (_wcsicmp(lpszValue, L"YELLOWGREEN") == 0) return clr = 0x32cd9a; return FALSE;
		}
		return FALSE;
	case 12:
		switch (ch)
		{
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LIGHTSKYBLUE") == 0) return clr = 0xface87;
			if (_wcsicmp(lpszValue, L"LEMONCHIFFON") == 0)   return clr = 0xcdfaff;
			return FALSE;
		case _T('M'):
			if (_wcsicmp(lpszValue, L"MEDIUMORCHID") == 0) return clr = 0xd355ba;
			if (_wcsicmp(lpszValue, L"MEDIUMPURPLE") == 0) return clr = 0xdb7093;
			if (_wcsicmp(lpszValue, L"MIDNIGHTBLUE") == 0) return clr = 0x701919;
			return FALSE;
		case _T('D'): if (_wcsicmp(lpszValue, L"DARKSEAGREEN") == 0) return clr = 0x8fbc8f; return FALSE;
		case _T('A'): if (_wcsicmp(lpszValue, L"ANTIQUEWHITE") == 0) return clr = 0xd7ebfa; return FALSE;
		}
		return FALSE;
	case 13:
		switch (ch)
		{
		case _T('D'):
			if (_wcsicmp(lpszValue, L"DARKSLATEBLUE") == 0) return clr = 0x8b3d48;
			if (_wcsicmp(lpszValue, L"DARKSLATEGRAY") == 0) return clr = 0x4f4f2f;
			if (_wcsicmp(lpszValue, L"DARKGOLDENROD") == 0) return clr = 0x0b86b8;
			if (_wcsicmp(lpszValue, L"DARKTURQUOISE") == 0) return clr = 0xd1ce00;
			return FALSE;
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LIGHTSEAGREEN") == 0) return clr = 0xaab220;
			if (_wcsicmp(lpszValue, L"LAVENDERBLUSH") == 0) return clr = 0xf5f0ff;
			return FALSE;
		case _T('P'):
			if (_wcsicmp(lpszValue, L"PALEGOLDENROD") == 0) return clr = 0xaae8ee;
			if (_wcsicmp(lpszValue, L"PALETURQUOISE") == 0) return clr = 0xeeeeaf;
			if (_wcsicmp(lpszValue, L"PALEVIOLETRED") == 0) return clr = 0x9370db;
			return FALSE;
		}
		return FALSE;
	case 14:
		switch (ch)
		{
		case _T('B'): if (_wcsicmp(lpszValue, L"BLANCHEDALMOND") == 0) return clr = 0xcdebff; return FALSE;
		case _T('C'): if (_wcsicmp(lpszValue, L"CORNFLOWERBLUE") == 0) return clr = 0xed9564; return FALSE;
		case _T('D'): if (_wcsicmp(lpszValue, L"DARKOLIVEGREEN") == 0) return clr = 0x2f6b55; return FALSE;
		case _T('L'):
			if (_wcsicmp(lpszValue, L"LIGHTSLATEGRAY") == 0) return clr = 0x998877;
			if (_wcsicmp(lpszValue, L"LIGHTSTEELBLUE") == 0) return clr = 0xdec4b0;
			return FALSE;
		case _T('M'): if (_wcsicmp(lpszValue, L"MEDIUMSEAGREEN") == 0) return clr = 0x71b33c; return FALSE;
		}
		return FALSE;
	case 15:
		if (_wcsicmp(lpszValue, L"MEDIUMSLATEBLUE") == 0) return clr = 0xee687b;
		if (_wcsicmp(lpszValue, L"MEDIUMTURQUOISE") == 0) return clr = 0xccd148;
		if (_wcsicmp(lpszValue, L"MEDIUMVIOLETRED") == 0) return clr = 0x8515c7;
		return FALSE;
	case 16:
		if (_wcsicmp(lpszValue, L"MEDIUMAQUAMARINE") == 0) return clr = 0xaacd66;
		return FALSE;
	case 17:
		if (_wcsicmp(lpszValue, L"MEDIUMSPRINGGREEN") == 0) return clr = 0x9afa00;
		return FALSE;
	case 20:
		if (_wcsicmp(lpszValue, L"LIGHTGOLDENRODYELLOW") == 0) return clr = 0xd2fafa;
		return FALSE;
	}
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertTextDecorations(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertFontWeight(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertFontQuality(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertLength(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertFontStyle(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertVerticalAlignment(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertHorizontalAlignment(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertTextWrapping(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertTextTrimming(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertTextAlignment(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertOrientation(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertBaselineAlignment(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertCursor(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertVisibility(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertContent(CXTPMarkupContext* pContext, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		CXTPMarkupRun* pRun = MARKUP_CREATE(CXTPMarkupRun, pContext);
		pRun->SetText((CXTPMarkupString*)pObject);
		MARKUP_ADDREF(pObject);

		CXTPMarkupTextBlock* pTextBlock = MARKUP_CREATE(CXTPMarkupTextBlock, pContext);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertScrollBarVisibility(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
...
CXTPMarkupObject* CXTPMarkupBuilder::ConvertStretch(CXTPMarkupContext* /*pContext*/, CXTPMarkupObject* pObject)
{
	if (IsStringObject(pObject))
	{
		LPCWSTR lpszValue = *((CXTPMarkupString*)pObject);
