...
void CSQLConnection::DisplayInfo(SQLRETURN RetCode, WCHAR *pDesc)
{
	...
	if (sqlReturn == SQL_SUCCESS)
	{
		...
		if (Error == WSAECONNRESET || 
			wcscmp(SqlState, L"08S01") == 0 ||	// 끊어진상태
			wcscmp(SqlState, L"24000") == 0)	// 잘못된 커서 상태
		{
			Disconnect();	// 끊고 다시 재연결
		}
	}
	else
	{
...
{
	if (m_hstmt == 0) return Reconnect();	// 끊어진상태면 재연결하쟈
	return ERROR_NONE;
}
...
int CSQLConnection::CommonRowcountQuery(WCHAR *pQuery)
{
	...
	if (CheckRetCode(RetCode, L"SQLPrepare") == ERROR_NONE){
		RetCode = SQLBindParameter (m_hstmt, 1, SQL_PARAM_OUTPUT, SQL_INTEGER, SQL_INTEGER, sizeof(int), 0, &nResult, sizeof(int), &cblen);	// @@rowcount 값을 말함

		RetCode = SQLExecute (m_hstmt);
		if (CheckRetCode(RetCode, L"SQLExecute") == ERROR_NONE)
		{			
			if (nResult == ERROR_NONE_ROWCOUNT) return ERROR_NONE;	// 성공
			// return ERROR_NONE;
			return nResult;			
		}
	}	
