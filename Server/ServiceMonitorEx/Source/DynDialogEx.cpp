...
int CDynDialogEx::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if (m_pFont == NULL) {
    ...
    wcsncpy_s(LogFont.lfFaceName, _T("MS Sans Serif"), LF_FACESIZE);
    LogFont.lfHeight = 8;
...
INT_PTR CDynDialogEx::DoModal() 
{
  ...
  CStringW aFontName = T2W(LogFont.lfFaceName);
  int nFontNameLen = aFontName.GetLength() * sizeof(WCHAR);

/*
  int cWC = MultiByteToWideChar(CP_ACP, 0, LogFont.lfFaceName, -1, NULL, 0);	// b4nfter 변경
...
*/
  ...
  CStringW aBoxCaption = T2W(m_strCaption.GetBuffer());
  int szBoxLen = aBoxCaption.GetLength() * sizeof(WCHAR);

/*
  cWC = MultiByteToWideChar(CP_ACP, 0, m_strCaption, -1, NULL, 0);	// b4nfter 변경
  int szBoxLen = cWC + 1;
...
*/
  ...
  if (hLocal != NULL) {
    ...
    *(WORD*)(pdest + 1) = 0;								// use default window class -- WORD to say it is 0 bytes
    pdest += sizeof(WORD);									// Increment
    memcpy(pdest, aBoxCaption.GetBuffer(), szBoxLen);		// Caption
    pdest += szBoxLen;

    *(WORD*)pdest = m_wFontSize;							// font size
    pdest += sizeof(WORD);
    memcpy(pdest, aFontName.GetBuffer(), nFontNameLen);		// font name
    pdest += nFontNameLen;
...
//     delete [] szBoxCaption;	// b4nfter 변경
//     delete [] szFontName;	// b4nfter 변경
    return iRet;
  }
  else {
