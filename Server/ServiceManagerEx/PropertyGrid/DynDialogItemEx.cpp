...
//char* CDynDialogItemEx::GetClassNameByType(DLGITEMTEMPLATECONTROLS TypeControl)	// b4nfter 변경
{
  switch(TypeControl) {
case BUTTON:
  return _T("BUTTON");
...
{
  pdest = (BYTE*)(((DWORD_PTR)pdest + 3) & ~3);  // make the pointer DWORD_PTR aligned		// b4nfter 변경
//  pdest = (BYTE*)(((DWORD)pdest + 3) & ~3);  // make the pointer DWORD aligned
  ...
  *(WORD*)pdest = (USHORT)m_eTypeControl;    // atom value for the control
  ...
  USES_CONVERSION;
	...
	CStringW aCaption = T2W(m_strCaption.GetBuffer());
	pdest += aCaption.GetLength() * sizeof(WCHAR);

//   WCHAR*  pchCaption;		// b4nfter 변경
