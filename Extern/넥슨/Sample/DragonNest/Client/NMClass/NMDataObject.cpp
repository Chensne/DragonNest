...
void VerifyGameCode( NMGameCode& uGameCode )
{
	...
	else if ( GET_GAMEMAINCODE( uGameCode ) == kMainGameCode_nexonus )
	{
		...
		case kSubGameCode_kartrider:
		case kSubGameCode_karttest:
		case kSubGameCode_motoloco:
		case kSubGameCode_sugarrush:
		case kSubGameCode_nxopp:
		case kSubGameCode_combatarms:
			break;
		default:
...
{
	::StringCchCopy( this->szMemo, MEMO_SIZE, pszMemo );
}
...
CNMNoteInfo::CNMNoteInfo( CNMNoteInfo2 & rhs ) : CNMSerializable( 'ni', 1, 1 )
{
	...
	::StringCchCopy( this->szFromLoginID, LOGINID_SIZE, rhs.GetSender().szLoginID );
	::StringCchCopy( this->szFromNickName, NICKNAME_SIZE, rhs.GetSender().szNickname );

	::StringCchCopy( this->szMessage, NOTEMSG_SIZE, rhs.szMessage );
	::StringCchCopy( this->szArgument, NOTEARG_SIZE, rhs.szArgument);
	::StringCchCopy( this->szCustomData, NOTECUSTOM_SIZE, rhs.szCustomData );
}
...
: CNMSerializable('cr', 1, 1)
{
	::StringCchCopy( this->szLoginID, LOGINID_SIZE, pszLoginID );
	this->vkey.uGameCode		= gameCode;
	this->vkey.uVirtualIDCode	= uVirtualIDCode;
	::StringCchCopy( this->szNickname, NICKNAME_SIZE, pszNickname );
}
...
namespace CNMNoteInfoHelper
{
	...
	BOOL StringToCharacterList(LPCTSTR pszBuffer, NMCharacterList & characterList, NMGameCode gameCode)
	{
		...
		for ( CNMStringList::iterator i = aToken.begin(), z = aToken.end(); i != z; ++i )
		{
			...
			::StringCchCopy( character.szNickname, NICKNAME_SIZE, i->c_str() );
	...
	: CNMSerializable( 'cm', 1, 1 )
{
	...
	case CNMChannelMessage::Type_ErrorMessage:
		::StringCchCopy( this->szSenderName, sizeof( this->szSenderName ) / sizeof( TCHAR ), s_szNotice );
		break;
	case CNMChannelMessage::Type_SystemMessage:
		::StringCchCopy( this->szSenderName, sizeof( this->szSenderName ) / sizeof( TCHAR ), s_szSystem );
		break;
	default:
		::StringCchCopy( this->szSenderName, sizeof( this->szSenderName ) / sizeof( TCHAR ), (LPCTSTR)rhs.senderName );
		break;
	}

	this->szChatHeader[ 0 ]			= 0;
	this->szFontHeader[ 0 ]			= 0;
	::StringCchCopy( this->szChatMessage, sizeof( this->szChatMessage ) / sizeof( TCHAR ), (LPCTSTR)rhs.message );
...
{
	this->szLoginId[ 0 ]	= 0;
	::StringCchCopy( this->szNickName, sizeof( this->szNickName ) / sizeof( TCHAR ), (LPCTSTR)channelMemberInfo.name );
}
...
CNMGuildOnlineInfo& CNMGuildOnlineInfo::operator=( CNMGuildOnlineInfoEx& infoEx )
{
	...
	::StringCchCopy( this->szChatServerIpAddr, 32, infoEx.szChatServerIpAddr );
...
CNMGuildOnlineInfo::operator CNMGuildOnlineInfoEx( void )
{
	...
	::StringCchCopy( infoEx.szChatServerIpAddr, 32, this->szChatServerIpAddr );
	...
	, uIpAddr( 0 )
{
	...
	::StringCchCopy( this->szNickName, sizeof( this->szNickName ) / sizeof( TCHAR ), (LPCTSTR)rhs.name );
}
...
namespace
{
	static NMGuildCode GameCodeToGuildCode( NMGameCode uGameCode )
	{
		...
		case kGameCode_bigshot:				return kGuildCode_bigshot;
		case kGameCode_zera:				return kGuildCode_zera;
		case kGameCode_zeratest:			return kGuildCode_zeratest;
		case kGameCode_zeratest_internal:	return kGuildCode_zeratest_internal;
		case kGameCode_lunia:				return kGuildCode_lunia;
		case kGameCode_luniatest:			return kGuildCode_luniatest;
		case kGameCode_koongpa:				return kGuildCode_koongpa;
	...
	void ChannelInfoToChatRoomInfo( CNMClientChannelInfo const & clientChannelInfo, CNMChatRoomInfoEx & chatRoomInfo )
	{
		...
		::StringCchCopy( chatRoomInfo.szGuildTitle, GUILDNAME_SIZE, (LPCTSTR)clientChannelInfo.guildName );
		::StringCchCopy( chatRoomInfo.szRoomTitle, ROOMTITLE_SIZE, (LPCTSTR)clientChannelInfo.title );
		//	chatRoomInfo.uLimitLevel
		chatRoomInfo.uMaxSize		= clientChannelInfo.maxUserCount;
		chatRoomInfo.uFlag			= clientChannelInfo.flag;
		::StringCchCopy( chatRoomInfo.szPassword, ROOMPWD_SIZE, (LPCTSTR)clientChannelInfo.password );
...
CNMChatRoomInfo& CNMChatRoomInfo::operator=( CNMChatRoomInfoEx& infoEx )
{
	...
	::StringCchCopy( this->szClubTitle, GUILDNAME_SIZE, infoEx.szGuildTitle );
	::StringCchCopy( this->szRoomTitle, ROOMTITLE_SIZE, infoEx.szRoomTitle );
	this->uLimitLevel		= infoEx.uLimitLevel;
	this->uMaxSize			= infoEx.uMaxSize;
	this->uFlag				= infoEx.uFlag;
	::StringCchCopy( this->szPassword, ROOMPWD_SIZE, infoEx.szPassword );
	...
	ITERATE( NMGuildMemberLevelList, infoEx.aLevelInfo, iter )
	{
		switch( ( *iter ).uLevel )
		{
		case 1:	::StringCchCopy( this->szLevel1Name, GUILDLEVEL_SIZE, ( *iter ).szName ); break;
		case 2:	::StringCchCopy( this->szLevel2Name, GUILDLEVEL_SIZE, ( *iter ).szName ); break;
		case 3:	::StringCchCopy( this->szLevel3Name, GUILDLEVEL_SIZE, ( *iter ).szName ); break;
		case 4:	::StringCchCopy( this->szLevel4Name, GUILDLEVEL_SIZE, ( *iter ).szName ); break;
		case 5:	::StringCchCopy( this->szLevel5Name, GUILDLEVEL_SIZE, ( *iter ).szName ); break;
		default:	break;
		}
	}
...
CNMChatRoomInfo::operator CNMChatRoomInfoEx( void )
{
	...
	::StringCchCopy( infoEx.szGuildTitle, GUILDNAME_SIZE, this->szClubTitle );
	::StringCchCopy( infoEx.szRoomTitle, ROOMTITLE_SIZE, this->szRoomTitle );
	infoEx.uLimitLevel		= this->uLimitLevel;
	infoEx.uMaxSize			= this->uMaxSize;
	infoEx.uFlag			= this->uFlag;
	::StringCchCopy( infoEx.szPassword, ROOMPWD_SIZE, this->szPassword );
	infoEx.uOwnerIdCode		= this->uOwnerIdCode;
	infoEx.uStatus			= this->uStatus;
	infoEx.uRequestProcId	= this->uRequestProcId;

	liInfo.uLevel = 1; ::StringCchCopy( liInfo.szName, GUILDLEVEL_SIZE, this->szLevel1Name ); infoEx.aLevelInfo.push_back( liInfo );
	liInfo.uLevel = 2; ::StringCchCopy( liInfo.szName, GUILDLEVEL_SIZE, this->szLevel2Name ); infoEx.aLevelInfo.push_back( liInfo );
	liInfo.uLevel = 3; ::StringCchCopy( liInfo.szName, GUILDLEVEL_SIZE, this->szLevel3Name ); infoEx.aLevelInfo.push_back( liInfo );
	liInfo.uLevel = 4; ::StringCchCopy( liInfo.szName, GUILDLEVEL_SIZE, this->szLevel4Name ); infoEx.aLevelInfo.push_back( liInfo );
	liInfo.uLevel = 5; ::StringCchCopy( liInfo.szName, GUILDLEVEL_SIZE, this->szLevel5Name ); infoEx.aLevelInfo.push_back( liInfo );
...
{
	this->nCode = nCode;
	::StringCchCopy( this->szMessage, sizeof( this->szMessage ) / sizeof( TCHAR ), pszMessage );
}
...
{
	::StringCchCopy( this->szLoginServerIp, IPADDR_SIZE, szLoginServerIp );
	::StringCchCopy( this->szStatServerIp, IPADDR_SIZE, szStatServerIp );
}
...
{
	::StringCchCopy( this->szLoginServerIp, IPADDR_SIZE, rhs.szLoginServerIp );
	::StringCchCopy( this->szStatServerIp, IPADDR_SIZE, rhs.szStatServerIp );
}
...
namespace
{
	void ChatRoomInfoToChannelInfo( CNMChatRoomInfoEx const & chatRoomInfo, CNMClientChannelInfo & clientChannelInfo )
	{
		...
		clientChannelInfo.channelSN			= chatRoomInfo.uRoomSerialNo;
