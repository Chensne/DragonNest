...
NMLoginAuthReplyCode CNMCOClientObject::LoginAuth( LPCTSTR pszNexonID, LPCTSTR pszPassword, INT32 nRegionCode )
{
	CNMLoginAuthFunc func( pszNexonID, pszPassword, nRegionCode );
	CNMManager::GetInstance().CallNMFunc( &func );
	return (NMLoginAuthReplyCode) func.nErrorCode;
}
...
)
{
	if ( szText )
	{
		...
		if ( CNMManager::GetInstance().CallNMFunc( &func ) )
		{
			...
			{
				::StringCchCopy( szText, uTextSize, func.szText );
				return TRUE;
			}
		}
	}
