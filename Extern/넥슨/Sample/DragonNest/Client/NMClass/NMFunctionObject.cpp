...
{
	this->uGameCode				= uGameCode;
	::StringCchCopy( this->szPassport, sizeof( this->szPassport ) / sizeof( TCHAR ), pszPassport);
	this->uSupportFlag			= uSupportFlag;
...
//{
	if ( this->IsCalling() )
	{
		...
		this->uGameCode	= decoder.Decode4();
		this->szText	= decoder.DecodeString();
	}
	else
	{
	...
	: CNMFunc( kCode, 1 )
{
	...
	this->szPassport[0]		= 0;
	::StringCchCopy( this->szNexonID, sizeof( this->szNexonID ) / sizeof( TCHAR ), pszNexonID );
	::StringCchCopy( this->szPassword, sizeof( this->szPassword ) / sizeof( TCHAR ), pszPassword );
	this->bSecondary		= FALSE;
	this->nRegionCode		= nRegionCode;
	this->nErrorCode		= kLoginAuth_ServerFailed;
}
...
CNMCharacterLoginFunc::CNMCharacterLoginFunc( LPCTSTR szUserId, _UInt32_ uVirtualIdCode, INT32 nServerCode )
: CNMFunc( kCode, 1 )
{
	INIT_STRING2( szUserId, NICKNAME_SIZE );
	this->keyUser.uGameCode			= kGameCode_NULL;
...
CNMCharacterLogoutFunc::CNMCharacterLogoutFunc( LPCTSTR szUserId, _UInt32_ uVirtualIdCode, INT32 nServerCode )
: CNMFunc( kCode, 1 )
{
	INIT_STRING2( szUserId, NICKNAME_SIZE );
	this->keyUser.uGameCode			= kGameCode_NULL;
