...
void CRegProfile::LoadProfile()
{
	...
	for( i = 0; i < ( int )dwProfileCount; i++ )
	{
		...
		if( GetRegistryString( HKEY_CURRENT_USER, szProfile, "SVNUrlBuildSet", szString ) == ERROR_SUCCESS )
		{
			CurProfile.szSVNUrlBuildSet = szString;
		}
...
void CRegProfile::SaveProfile()
{
	...
	for( int i = 0; i < ( int )m_vecProfile.size(); i++ )
	{
		sprintf_s( szProfile, 1024, "%s\\Profile\\Profile %d", REG_SUBKEY, i );
		...
		SetRegistryString( HKEY_CURRENT_USER, szProfile, "SVNUrlBuildSet",( char * )m_vecProfile[ i ].szSVNUrlBuildSet.c_str() );
