...
BOOL CDnOptionData::SetOptionValue( FILE *stream, char * szString )
{
	...
	else if( strcmp( "Username", szString ) == 0 )
	{
		char uname[32];
		fscanf_s( stream, "%s", &(uname) );
		strcpy(username,uname);
		return TRUE;
	}
	else if( strcmp( "Password", szString ) == 0 )
	{
		char passwd[32];
		fscanf_s( stream, "%s", &(passwd) );
		strcpy(password,passwd);
		return TRUE;
	}
	else if( strcmp( "NewUI", szString ) == 0 )
	{
		int NewUI = 0;
		fscanf_s( stream, "%d", &(NewUI) );
		m_bNewUI = NewUI?true:false;
		return TRUE;
	}
#ifdef _USE_MULTILANGUAGE
	else if( strcmp( "LanguageID", szString ) == 0 )
	{
		...
		DNPATCHINFO.SetLanguageParamByLanguageID( m_nLanguageID );
