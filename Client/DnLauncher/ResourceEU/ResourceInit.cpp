...
void InitCtrlRect()
{
	int		nCurrentResourceID	= DNCTRLDATA.GetMulitiLanguageResourceID();
	CString	szFileName			= DNCTRLDATA.GetMulitiLanguageFilePath();
	DNCTRLDATA.SetControlDataFromResource(nCurrentResourceID/*IDR_XML_CONTROL_DATA*/, szFileName.GetBuffer(0)/*_T("ControlPositionList.xml")*/, g_CtrlRect, MAX_CTRL_NUM);
...
int GetLanguageResourceIndexOffset( CString strLanguageParam )	// Resource.h의 국가별 offset값을 전달
{
	int nIndexOffset = 0;
	if( strLanguageParam == L"FRA" )
		nIndexOffset = 10000;
	else if( strLanguageParam == L"ESP" )
		nIndexOffset = 20000;
	else if( strLanguageParam == L"GER" )
		nIndexOffset = 30000;
