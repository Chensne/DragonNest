		...
		)
	{
		...
		for( int i=0; i<nColumnCount; i++ ) {
			if( i == m_nColorColumnIndex ) {
				...
				sscanf_s( szStr.GetBuffer(), "%c|%d|%d", cStr, 2, &TextColor, &BGColor );

				Rect[i].left += 2, Rect[i].right -= 2; Rect[i].bottom -= 1;

				pDC->FillSolidRect( &Rect[i], BGColor );
				DrawFrameBorder( pDC, &Rect[i] );

				pDC->SetTextColor( RGB( 0, 0, 0 ) );
				pDC->DrawText( cStr, 1, &Rect[i], DT_CENTER | DT_VCENTER );

				pDC->SetTextColor( TextColor );
				Rect[i].left -= 1;	Rect[i].top -= 1;	Rect[i].right -= 1;	Rect[i].bottom -= 1;
				pDC->DrawText( cStr, i, &Rect[i], DT_CENTER | DT_VCENTER );
			}
			else {
	...
	{	
		...
		for( int i=0; i<nColumnCount; i++ ) {
			if( i == m_nColorColumnIndex ) {
				...
				char cStr[2] = { 0, };
				CString szStr = GetItemText( lpDrawItemStruct->itemID, m_nColorColumnIndex );
				sscanf_s( szStr, "%c|%d|%d", cStr, 2, &TextColor, &BGColor );

				pDC->FillSolidRect( &Rect[i], RGB(106, 36, 10) );
				Rect[i].left += 2, Rect[i].right -= 2; Rect[i].bottom -= 1;
