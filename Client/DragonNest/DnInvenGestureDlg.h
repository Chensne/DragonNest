...
class CDnInvenGestureDlg : public CDnCustomDlg
{
	struct SKILL_SLOT
	{
		...
		CEtUIStatic *m_pSkillNameBar;

		SKILL_SLOT()
			: m_pSkillBase(NULL)
			, m_pSlotButton(NULL)
			, m_pSkillName(NULL)
			, m_pSkillNameBar(NULL)
		{
		}
		...
		void InitSkillSlot()
		{
			...
			m_pSkillNameBar->SetTextureColor( 0x4CFFFFFF, 0 );
		};
		...
		void SetSkillSlot( int nGestureID, LPCWSTR wszName )
		{
			...
			m_pSkillNameBar->SetTextureColor( 0xFFFFFFFF, 0 );
	...
	enum
	{
		ITEMSLOT_MAX_SIZE = 16
	};

public:
	CDnInvenGestureDlg( UI_DIALOG_TYPE dialogType = UI_TYPE_FOCUS, CEtUIDialog *pParentDialog = NULL, int nID = -1, CEtUICallback *pCallBack = NULL );
	virtual ~CDnInvenGestureDlg(void);
