...
class CDnSkillRadioMsgDlg : public CDnCustomDlg
{
	struct SKILL_SLOT
	{
		CDnRadioMsgButton *m_pSlotButton;
		...
		void SetSkillSlot( int nRadioMsgID, LPCWSTR wszName )
		{
			if( nRadioMsgID == 0 ) return;

			m_pSkillBase->SetTextureColor( 0xFFFFFFFF, 0 );
			m_pSlotButton->SetRadioMsgInfo( nRadioMsgID );
...
public:
	CDnSkillRadioMsgDlg( UI_DIALOG_TYPE dialogType = UI_TYPE_FOCUS, CEtUIDialog *pParentDialog = NULL, int nID = -1, CEtUICallback *pCallBack = NULL );
	virtual ~CDnSkillRadioMsgDlg(void);
