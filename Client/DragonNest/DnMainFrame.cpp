...
// 瘤沥等 弃歹狼 窍困 颇老 府胶飘甫 沤祸秦辑 货酚霸 眠啊等 颇老捞 粮犁茄促搁 眠啊
...
bool CDnMainFrame::PreInitialize()
{
	...
	// 秦寇OS俊辑 老何 胶飘傅篮 力措肺 佬绢坷瘤 给窍扁锭巩俊 捞犯霸 风飘弃歹甫 喊档肺 罐酒具 犬角洒 茫阑 荐 乐促.
	...
	if( CEtResourceMng::GetInstance().IsUsePackingFile() ) {	// 菩欧颇老 府家胶 肺爹 角菩...
...
bool CDnMainFrame::PreInitializeDevice()
{
	...
	if( bResult ) {
		...
		//GetEtWater()->Optimize( true );	// 拱 可萍付捞令 老窜 馋促.
		...
		if( m_nWinVer >= 106 ) {		// Vista(106), Win7(107) 篮 DDraw 措脚 D3D9 狼 MemCheck 甫 荤侩.
...
//#if defined(_US) || defined(_TH) || defined(_ID) || defined(_RU) || defined(_EU)	// Word Break 荤侩窍绰 惫啊绰 眠啊且 巴
...
bool CDnMainFrame::ThreadInitialize()
{
	...
	//	ASSERT(false && "UIString_rlkt.xml 肺爹 俊矾");
	//	return false;
	//}
...
bool CDnMainFrame::Initialize()
{
	...
	// DeadLock Detect Task 积己
	DWORD dwDetectTime = 60000 * 10;
#if defined( PRE_FIX_CLIENT_FREEZING )
	dwDetectTime = 60000 * 2;
#endif

	m_pHangCrashTask = new CDnHangCrashTask( dwDetectTime ); // 5盒悼救 悼累绝栏搁 农贰浆
...
bool CDnMainFrame::Finalize()
{
	...
	// 叼滚弊 鸥胶农绰 NEw 俊辑 持篮霸 酒聪聪鳖 RemoveAllTask 窍扁傈俊 哗霖促.
...
//	_beginthreadex( NULL, 65536, TestThread2, NULL, 0, &TestThreadID2 );
	while(1)
	{
		...
		// _KR 惫郴呼靛俊急 炼钦 棺 牡叼单捞飘甫 流立 坊歹傅窍聪 捞犯霸 贸府.
...
bool CDnMainFrame::IMEProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	{
		// 器目胶啊 盲泼芒俊 啊乐瘤臼促搁 叼弃飘橇肺矫历俊档 焊郴瘤 臼绰促.
		bool bReturn = true;
		...
		if( !bReturn && wParam == IMN_OPENCANDIDATE )
		{
			...
			// 钱胶农赴锭绰 扩档快 母记阑 绊妨窍瘤 臼绊 弊 磊府俊 坊歹傅秦 盔贰 涝仿芒捞 啊妨柳促.
			// 弊贰辑 碍力肺 20侨伎 郴府档废 窍摆促.
			if( CGameOption::GetInstance().m_bWindow == false )
				pt.y += 20;

			// 父距 炼钦扩档快啊 凯妨乐促搁 困摹啊 般媚辑 啊妨滚府霸 瞪 巴捞促.
			// 炼钦扩档快啊 凯妨乐促搁 x绵栏肺档 剐绢初摆促.
			...
			if( ( hIMC = ImmGetContext( hWnd ) ) )
			{
				...
				// 哪腔磐付促 鞘夸 牢磊啊 促弗 淀 钦聪促...
				form.dwStyle = CFS_FORCE_POSITION | CFS_CANDIDATEPOS;

				// 促澜 滴 临篮 盔窍绰 Candidiate Window狼 谅钎涝聪促.
				form.ptCurrentPos.x = pt.x;
				form.ptCurrentPos.y = pt.y;

				// 拌加 抛胶飘 秦焊聪 牡叼单捞飘甫 流立 框流捞绰 巴焊促 哪器瘤记 扩档快甫 捞悼矫难 贸府窍绰霸
				// 芒 困摹 棱绰单 乐绢辑 歹 亮篮 芭 鞍促.
				// 父距 唱吝俊 促矫 捞 累诀阑 窍霸 等促搁 哪器瘤记 扩档快狼 困摹甫 利例窍霸 棱绊 唱赣瘤绰 舅酒辑 API俊 该扁绰霸 唱阑 芭 鞍促.
...
void CDnMainFrame::TemporaryKeyProc()
{
	...
	{
		GetInterface().ProgressSimpleMsgBox(GetEtUIXML().GetUIString(CEtUIXML::idCategory1, 3450), 5.f, true, UI_DISABLE_DIALOG, NULL); // UISTRING : 颇萍 犁立加吝...
	}
...
//							CDnActor::s_hLocalActor->GetMatEx()->m_vPosition -= CDnActor::s_hLocalActor->GetMatEx()->m_vZAxis * 100.f;
	...
	if( GetAsyncKeyState( VK_MENU ) < 0 && // CTRL+ALT+D  -> DebugBreak	// 角青档吝 癌磊扁 宏饭捞农 吧绊 酵阑锭 荤侩.
...
{
// 捞力绰 JP档 扩档快 郴厘 IME甫 荤侩窍扁锭巩俊 捞 规侥栏肺 贸府茄促.
	...
	// 酒贰 函荐贸府啊 绝栏搁 扩档快 努扼捞攫飘 康开阑 穿弗盲 鸥捞撇肺 付快胶公宏矫 扩档啊 捞悼登霸 等促.
...
*/
	...
	case WM_KEYDOWN:
	{
			...
			// UI 绝绰 惑怕俊辑 菩靛肺 狞浇吩 贸府窍绰 何盒篮 绢驴荐 绝捞 捞犯霸 茄促.
			// GetKeyState蔼 贸府窍绰 苞沥俊辑 菩靛绰 流立 虐甫 涝仿窍绊 乐绰霸 酒聪扼辑 贸府啊 救登扁 锭巩捞促.
			if( CDnInterface::GetInstancePtr() )
			...
			if( GetAsyncKeyState( VK_MENU ) < 0 && // CTRL+ALT+0  -> QA 傈侩 丑厘 备开焊咯淋
				GetAsyncKeyState( VK_CONTROL ) < 0 && 
				GetAsyncKeyState( VK_UP ) < 0 ) 
			{
				if( CDnInterface::GetInstancePtr() )
				{
#if defined(_WORK)	// verygoodd21 傈侩 摹飘虐
			...
			if( GetAsyncKeyState( VK_TAB ) < 0 )
			{
				if( GetAsyncKeyState( VK_UP ) < 0 )
				{
					...
					swprintf_s( wszString, _countof( wszString ), L"泅犁 Blur Weight : %1f", fBlurWeight );
				...
				else if( GetAsyncKeyState( VK_LEFT ) < 0 )
				{
					...
					WCHAR wszString[256] = {0,};
					swprintf_s( wszString, _countof( wszString ), L"泅犁 利侩 迄飘农扁 : %d", nMinimizeSize );
					GetInterface().AddChatMessage( CHATTYPE_SYSTEM, L"", wszString );
				}
				...
				case CEtBCLEngine::ErrorNotEnoughDiskSpace:
					{
						...
						if( nFreeByte < 1048576 ) { // 1皋啊 捞窍老锭父 俊矾贸府窍磊.
							nErrorStrIndex = 6109;
						}
					}
					break;
	...
	case WM_NCHITTEST:
		{
#if defined(_CH) && defined(_AUTH)
			// OA芒 昆其捞瘤锭巩俊 鸥捞撇 棱绊 扩档快 捞悼矫虐绰 风凭 荤侩窍瘤 臼绰促.
			break;
#endif
		...
		break;
	case WM_CONTEXTMENU:	// 母记官俊促啊 付快胶 坷弗率 滚瓢 穿甫锭
	case 0x313:				// 累诀钎矫临俊辑 付快郊 坷弗率 滚瓢 穿甫锭
		return 1;
	}
	...
	if( !pTask && EtInterface::StaticMsgProc( hWnd, message, wParam, lParam ) )
	{
		if( bMsgProc ) {
			...
			// 咯扁辑 沥富 府畔且 巴甸父 府畔窍霸 窍磊..
			switch( message ) {
				// WM_IME_COMPOSITION栏肺 坷绰 臂磊甸捞 滴锅究 嘛洒绰 巩力锭巩俊 咯扁辑 捞犯霸 府畔窍档废 登绢乐促.
				// IMEEditBox::SendCompString 俊辑 流立 皋技瘤甫 朝府绰 备炼扼, 咯扁辑 府畔阑 秦具 滴锅 贸府登瘤 臼绰 巴捞促.
				case WM_IME_COMPOSITION:
				// 流立 坊歹傅窍绰 IME甫 荤侩窍绰 档吝俊 扩档快侩 IME芒捞 拳搁 惑窜俊 弊妨柳促搁,
				// 贸府登瘤 臼酒具且 IME包访 皋技瘤甸捞 叼弃飘橇肺矫历肺 艾阑 犬伏捞 臭促.
				// 泅犁 CH, JP笛促 寇何 IME甫 荤侩窍扁锭巩俊 贸府且 鞘夸 绝栏唱, 唱吝俊 鞘夸窍搁 林籍 钱绊 return 1 窍搁 瞪 巴捞促.
...
bool CDnMainFrame::CheckRecordDiskSpace( bool bBegin )
{
	...
	if( bBegin ) {
		...
		if( nFreeByte >= 1073741824 ) { // 扁啊啊 逞绢啊搁 GB 窜困肺.
...
void CDnMainFrame::RecordEnd()
{
	...
	double fRecordSize;
	bool bGigaUnit = false;
	if( nRecordSize >= 1073741824 ) { // 扁啊啊 逞绢啊搁 GB 窜困肺.
