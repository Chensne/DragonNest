...
CDnCashInventory::CDnCashInventory(void)
{
}
...
void CDnCashInventory::CreateCashItem( TItem &item, int nSlotIndex )
{
	if( !m_pInvenDlg )
	{
...
bool CDnCashInventory::InsertItem( CDnItem *pItem, bool bMoving )
{
	if (pItem == NULL)
	{
		_ASSERT(0 && "CDnCashInventory::InsertItem - ITEM is NULL\n");
		return false;
	}

	if( !m_pInvenDlg )
	{
		CDebugSet::ToLogFile( "CDnCashInventory::InsertItem, m_pInvenDlg is NULL!" );
		return false;
	}
...
bool CDnCashInventory::RemoveItem( int nSlotIndex )
{
...
void CDnCashInventory::SetUsableSlotCount( int nCount )
{ 
...
void CDnCashInventory::DecreaseDurability( int nValue )
{
...
void CDnCashInventory::DecreaseDurability( float fValue )
{
	INVENTORY_MAP_ITER iter = m_mapInventory.begin();
	INVENTORY_MAP_ITER iterEnd = m_mapInventory.end();
