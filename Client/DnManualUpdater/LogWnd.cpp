...
//-------------------------------------------------------------------------------------------------------------
__LogWnd::__LogWnd( bool bAttachDate /* = false */, DWORD nTarget /* = LOG_TARGET_WINDOW */, const TCHAR* szFilename /* = NULL */, const TCHAR* szPath /* = NULL */  )
{
...
#ifdef _CH
			_sntprintf_s(m_szFilename, 4000, _T("%s%s\\Log\\%s_%04d%02d%02d_%02d%02d%02d_Log.txt"),
				__szDrive, __szDir, __szFileName,
				1900+now.tm_year, now.tm_mon+1, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec);
#else //_CH
			_sntprintf_s(m_szFilename, 4000, _T("%s%s\\Log\\%s_%04d년%02d월%02d일_%02d시%02d분%02d초_Log.txt"),
				__szDrive, __szDir, __szFileName,
				1900+now.tm_year, now.tm_mon+1, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec);
#endif //_CH
		}
...
int __LogWnd::Log( int nLogLevel, const TCHAR* szLog )
{
	...
	if ( m_bAttachDate )
	{
		_sntprintf_s(szTotal, 2048, _T("%04d년%02d월%02d일%02d시%02d분%02d초,%s,\t%s"),
