...
public:
	...
	// ----- MUTATORS ---------------------
	void Setup(NavigationMesh* Parent, const EtVector3& StartPoint, NavigationCell* StartCell, const EtVector3& EndPoint, NavigationCell* EndCell);
	void AddWayPoint(const EtVector3& Point, NavigationCell* Cell);
	void EndPath();
	...
	WAYPOINT_LIST&			LineOfSightWayPointList();
...
private:
	...
	WAYPOINT_LIST		m_FurthestWaypointList;
...
------------------------------------------------------------------------------------------
*/
inline NavigationPath::NavigationPath() : m_Parent( NULL )
{
}
...
//
//-------------------------------------------------------------------------------------://
inline void NavigationPath::Setup(NavigationMesh* Parent, const EtVector3& StartPoint, NavigationCell* StartCell, const EtVector3& EndPoint, NavigationCell* EndCell)
{
...
//
//-------------------------------------------------------------------------------------://
inline void NavigationPath::AddWayPoint(const EtVector3& Point, NavigationCell* Cell)
{
	WAYPOINT NewPoint;
...
inline WAYPOINT_LIST& NavigationPath::LineOfSightWayPointList()
{
	m_FurthestWaypointList.clear();

	WayPointID it = m_WaypointList.begin();
	if ( !m_WaypointList.empty() )
	{
		m_FurthestWaypointList.push_back(*it);

		for ( ; it != m_WaypointList.end() ; it++ )
		{
			it = GetFurthestVisibleWayPoint(it);

			if ( it == m_WaypointList.end() )
				break;
