...
int gpiro_cmpfunc(const void* a, const void* b)
{
    ...
    return (int)p1->GetClientData() - (int)p2->GetClientData();
}

wxArrayPGProperty GetPropertiesInRandomOrder( wxPropertyContainerMethods* props, int iterationFlags = wxPG_ITERATE_ALL )
{
...
void FormMain::RunTests( bool fullTest )
{
#if TESTING_WXPROPERTYGRIDADV
    wxString t;
...
#define FLAG_TEST_SET2 (wxSTAY_ON_TOP|wxCAPTION|wxICONIZE|wxSYSTEM_MENU)
...

    {
        ...
        for ( i=0; i<3; i++ )
        {
            ...
            pgman->SaveEditableState(&state);
            wxString stringState = state.GetAsString();
            wxPGMEditableState state2;
            bool res = state2.SetFromString(stringState);
            if ( res )
                pgman->RestoreEditableState(state2);
            else
                RT_FAILURE();
        }
    }
