  ...
  for (k1=0;k1<nfm1;k1++){
    ...
    for (j=0;j<ipm;j++){
      ...
      for (ii=2;ii<ido;ii+=2){
        ...
        arg=fi*argld;
        wa[i++]=FMOD_ogg_cos(arg);
        wa[i++]=FMOD_ogg_sin(arg);
      }
      is+=ido;
    }
    l1=l2;
...
static void dradfg(int ido,int ip,int l1,int idl1,float *cc,float *c1,
  ...
  arg=tpi/(float)ip;
  dcp=FMOD_ogg_cos(arg);
  dsp=FMOD_ogg_sin(arg);
  ipph=(ip+1)>>1;
  ipp2=ip;
  idp2=ido;
  nbd=(ido-1)>>1;
...
void drft_init(void *context, drft_lookup *l,int n){
  l->n=n;
...
void drft_clear(void *context, drft_lookup *l){
  if(l){
